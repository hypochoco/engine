...
// Quaternion (w, x, y, z) → continuous 6D rotation (first two rotation-matrix columns). MUST match
// `sim1/tasks/proprio.py::quat_to_6d` so a `rotation=sixd` policy sees the same obs it trained on.
...
struct PolicyNet {
    ...
    static PolicyNet load(const std::string& path) {
        ...
        const int version = (tag == "SIM1_POLICY_V3") ? 3 : (tag == "SIM1_POLICY_V2") ? 2
