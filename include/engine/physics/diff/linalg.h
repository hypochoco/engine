...
//
//  Every operation is annotated ENGINE_HD (host+device under nvcc, no-op on host) so the SAME
//  templated math runs in the batched CUDA kernel — see hd.h.
//
...
template <class S> ENGINE_HD V3<S> operator+(const V3<S>& a, const V3<S>& b) { return { a.x + b.x, a.y + b.y, a.z + b.z }; }
template <class S> ENGINE_HD V3<S> operator-(const V3<S>& a, const V3<S>& b) { return { a.x - b.x, a.y - b.y, a.z - b.z }; }
template <class S> ENGINE_HD V3<S> operator*(const S& s, const V3<S>& a) { return { s * a.x, s * a.y, s * a.z }; }
template <class S> ENGINE_HD S dot(const V3<S>& a, const V3<S>& b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
template <class S> ENGINE_HD V3<S> cross(const V3<S>& a, const V3<S>& b) {
    return { a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x };
}

template <class S> ENGINE_HD M3<S> skew(const V3<S>& v) {
    M3<S> r;
...
template <class S> ENGINE_HD M3<S> identity3() {
    M3<S> r; for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) r.m[i][j] = S(i == j ? 1 : 0); return r;
}
template <class S> ENGINE_HD M3<S> operator*(const M3<S>& a, const M3<S>& b) {
    M3<S> r;
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) { S s = S(0); for (int k = 0; k < 3; ++k) s = s + a.m[i][k] * b.m[k][j]; r.m[i][j] = s; }
    return r;
}
template <class S> ENGINE_HD V3<S> operator*(const M3<S>& a, const V3<S>& v) {
...
template <class S> ENGINE_HD M3<S> scaled(const M3<S>& a, const S& s) {
    M3<S> r; for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) r.m[i][j] = a.m[i][j] * s; return r;
}
template <class S> ENGINE_HD M3<S> transpose(const M3<S>& a) {
...
template <class S> ENGINE_HD M3<S> rodrigues(const V3<S>& axis, const S& angle) {
...
template <class S> ENGINE_HD V3<S> ang(const V6<S>& v) { return { v.d[0], v.d[1], v.d[2] }; }
template <class S> ENGINE_HD V3<S> lin(const V6<S>& v) { return { v.d[3], v.d[4], v.d[5] }; }
template <class S> ENGINE_HD V6<S> makeV6(const V3<S>& w, const V3<S>& v) { return { { w.x, w.y, w.z, v.x, v.y, v.z } }; }

template <class S> ENGINE_HD void setBlock(M6<S>& M, int r0, int c0, const M3<S>& B, bool neg = false) {
    for (int i = 0; i < 3; ++i) for (int j = 0; j < 3; ++j) M.m[r0 + i][c0 + j] = neg ? -B.m[i][j] : B.m[i][j];
}
// Motion transform [[E,0],[−E·skew(r), E]].
template <class S> ENGINE_HD M6<S> plux(const M3<S>& E, const V3<S>& r) {
    M6<S> X; setBlock(X, 0, 0, E); setBlock(X, 3, 3, E); setBlock(X, 3, 0, E * skew(r), true); return X;
}
// Motion cross [[skew(ω),0],[skew(v), skew(ω)]].
template <class S> ENGINE_HD M6<S> crm(const V6<S>& v) {
...
template <class S> ENGINE_HD V6<S> operator+(const V6<S>& a, const V6<S>& b) {
    V6<S> r; for (int i = 0; i < 6; ++i) r.d[i] = a.d[i] + b.d[i]; return r;
}
template <class S> ENGINE_HD V6<S> scaled(const V6<S>& a, const S& s) {
    V6<S> r; for (int i = 0; i < 6; ++i) r.d[i] = a.d[i] * s; return r;
}
template <class S> ENGINE_HD S dot(const V6<S>& a, const V6<S>& b) { S s = S(0); for (int i = 0; i < 6; ++i) s = s + a.d[i] * b.d[i]; return s; }
template <class S> ENGINE_HD M6<S> outerScaled(const V6<S>& a, const V6<S>& b, const S& s) {   // (a bᵀ)·s
...
template <class S> ENGINE_HD V3<S> lift(const V3<double>& v) { return { S(v.x), S(v.y), S(v.z) }; }
template <class S> ENGINE_HD M3<S> lift(const M3<double>& a) {
...
template <class S> ENGINE_HD M3<S> expSO3(const V3<S>& w) {
...
template <class S> ENGINE_HD void invertSmall(const S* A, int n, S* inv) {
...
template <class S> ENGINE_HD V6<S> solveM6(const M6<S>& A, const V6<S>& b) {
    S M[6][7];
...
template <class S> ENGINE_HD V3<S> vee(const M3<S>& M) {
...
// Host-only (finite-difference / jacobian tooling) — not on the device hot path, so left un-annotated.
...
template <class S> ENGINE_HD V6<S> liftV6(const V6<double>& v) { V6<S> r; for (int i = 0; i < 6; ++i) r.d[i] = S(v.d[i]); return r; }
